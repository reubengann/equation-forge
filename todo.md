...
- If `f(x)` is a function `f` of `x`, we don't want `(f(x))^2` to be interpreted by the editor as `f^2 x^2` for the purposes of evaluation, factoring, etc. However, there is no way to represent this in latex; there simply is not such concept. Moreover, there is no suitable way of metadata tagging symbols to be interpreted that way. Therefore, we should provide a manual way to temporarily tag symbols that should be interpreted as functions within the AST. This data persists as long as the equation lives in the UI. When exporting, we don't attempt to exfiltrate this data. If someone copies an equation out, and then pastes it back in from jupyter or whatever, they have to tag those symbols again. Tagged symbols to be interpreted as functions must be followed by a display_group that are parentheses, which is interpreted as comma delimited list of expressions. Valid inputs are rather general
  - f(x)
  - g(x, y)
  - h(x+2, f(y))
  - f(x=2)
    I suppose we should keep these as immutable_expressions, as there's not much call to rearrange terms in them. If you want to substitute, you should probably substitute the entire entity. The data model of an equation expands to have a parallel `functionSymbols` array storing symbols that represent functions, along with their nodeIds. When rendered, make any symbol tagged as a function have a slightly different color/opacity from a normal one. When a single, symbol-like selection has been made, which is followed by a valid display_group, a button will exist to toggle function symbol on/off.
