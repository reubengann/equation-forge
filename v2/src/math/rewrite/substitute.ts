...
export function getSubstitutionSelection(
  ...
  if (selection.kind === "single") {
    ...
    if (!expr) return null;
    return { expr: cloneExpr(expr), latex: exprToLatex(expr, false) };
  }

  const range = resolveMultiSelectionRange(document, selection);
  if (!range) return null;
  return { expr: cloneExpr(range.selectedExpr), latex: exprToLatex(range.selectedExpr, false) };
}
...
function resolveMultiSelectionRange(
  ...
  const indexedChildren = selection.nodeIds
    .map((nodeId) => {
      ...
      return { nodeId, index: location.index };
    })
    .filter((child): child is { nodeId: string; index: number } => child !== null)
